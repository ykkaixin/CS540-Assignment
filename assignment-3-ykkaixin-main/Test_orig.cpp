...
run(void *vp) {
        ...
        switch (act) 
        {
            case 0:
                // Assign ptr new TestObj;
                {
                    ...
                    if (Table[i].ptr != 0) 
                    {
                        //printf("%ld: Table[%d] resets ..\n", (long int) tid, i);
                        Table[i].ptr -> reset(new TestObj); // fix
                        //printf("%ld: Table[%d] reset done\n", (long int) tid, i);
            ...
            case 2:
                // Assign ptr from another ptr
                {
                    ...
                    {
                        ec = pthread_mutex_lock(&Table[i].lock); assert(ec == 0);
                        if (i != j) ec = pthread_mutex_lock(&Table[j].lock); assert(ec == 0);
                    } 
                    else 
                    {
                    ...
                    ec = pthread_mutex_unlock(&Table[i].lock); assert(ec == 0);
                    if (i != j) ec = pthread_mutex_unlock(&Table[j].lock); assert(ec == 0);
            ...
            case 3:
                // Create new sptr.
                {
                    ...
                    //if (rand(0, 0) == 1)
                    {
                        ...
                        if (!Table[i].ptr) 
                        {
                            //printf("Table[%d] adds new ..\n", i);
                            //printf("%ld: Table[%d] creates ..\n", (long int) tid, i);
                            Table[i].ptr = new SharedPtr <TestObj>; // const is called implicitly or not ?
                    ...
                    else 
                    {
                        ...
                        //printf("Table[%d]-[%d] copied ..\n", i, j);

                        if (!Table[i].ptr && Table[j].ptr) 
                        {
                            //printf("%ld: Table[%d] [%d] copied ..\n", (long int) tid, i, j);
                            Table[i].ptr = new SharedPtr <TestObj> (* Table[j].ptr); // copy const
                            counters.new_copy++;
                        }
    ...
    double rs = RunSecs;
    printf("%10lu (ops): assignment_new = %lu, assignment = %lu, reset = %lu,"
        " new_default = %lu, new_cop = %lu, delete = %lu\n"
        "    rates (ops/sec): assignment_new = %.0f, assignment = %.0f, reset = %.0f,"
        " new_default = %.0f, new_copy = %.0f, delete = %.0f\n\n",
     (unsigned long) tid,
     counters.assignment_new, counters.assignment, counters.reset, 
     counters.new_default, counters.new_copy, counters.delet,
     double(counters.assignment_new)/rs, double(counters.assignment)/rs, 
     double(counters.reset)/rs, double(counters.new_default)/rs, double(counters.new_copy)/rs, double(counters.delet)/rs);
...
threaded_test() {
    ...
    //Table[0].ptr = new SharedPtr <TestObj>; // Default const
    //SharedPtr <TestObj> * ptr = new SharedPtr <TestObj> ();
    //printf("\n");

    //Table[1].ptr = new SharedPtr <TestObj> (* Table[0].ptr); // Copy const
    //SharedPtr <TestObj> * ptr_2 = new SharedPtr <TestObj> (* ptr);
    //printf("\n");
